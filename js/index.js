var _height=$(window).height();//获取当前窗口的高度
$('#focusBar').css('height',_height+'px');//调整列表的宽高
...
function changeFocus(dir) {
	...
	if(dir) {
		...
		$("#focusIndex"+currentFocusI+" .focusR").animate({left: -($(window).width()/2+1000)},900,'easeInExpo',function(){
				$("#focusIndex"+nextI+" .focusL").animate({left: -640},1200,'easeInOutCirc');
				$("#focusIndex"+nextI+" .focusR").animate({left: -640},900,'easeInOutCirc');
				$("#focusIndex"+nextI+" .zhezhao").animate({left: -710},1400,'easeInOutCirc');
